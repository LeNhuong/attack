from attackcoin.globaldef.hash import HASH_SIZE
from attackcoin.network.attacksocket import ATTACKSocket
from attackcoin.network.protocol import Protocol
from attackcoin.objects.xstate import BlockError
from attackcoin.atnode.workers.addressprocessor import AddressProcessor
from attackcoin.atnode.workers.blockprocessor import BlockProcessor
from attackcoin.atnode.workers.broadcaster import Broadcaster
from attackcoin.atnode.workers.listener import Listener
from attackcoin.atnode.workers.peeradder import PeerAdder
from attackcoin.atnode.workers.branchbuilder import BranchBuilder
from attackcoin.atnode.node import Node
    ...
    def run(self):
        ...
        if not self.node.unconnected_addrs:
            ...
            while True:
                ...
                # initialize a socket
                s = ATTACKSocket(virt_loc=self.node.virt_loc, name=self.node.name)
        ...
        if self.node.verbose:
            ...
            print("Current block reward: {} BLC.".format(
                latest_state.reward))
        ...
        cmd = "global LazyMiner; " \
              "from attackcoin.atnode.workers.miners.{} import LazyMiner".format(
