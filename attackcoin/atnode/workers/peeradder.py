...
from attackcoin.atnode.workers.server import Server
...
class PeerAdder(Thread):
    ...
    def run(self):
        while True:
            ...
            self.node.peers_smp.acquire()
            s1 = ATTACKSocket(virt_loc=self.node.virt_loc, name=self.node.name)
            s2 = ATTACKSocket(virt_loc=self.node.virt_loc, name=self.node.name)
